...
all <- read.structure("data/rawdata/Microsat.adults.plus.unrelated.chicks.forstructure.stru", 
...
We tested for Hardy-Weinberg in the combined dataset consisting of adults
and unrelated chicks (one chick per brood).

```{r hardyweinberg, warning=FALSE, message=FALSE, include=FALSE}

#### Testing for Hardy-Weinberg equilibrium ####

# First all together
allHWE.all <- pegas::hw.test(all, B = 1000) 
...
```
```{r hardyweinberg, warning=FALSE, message=FALSE, include=TRUE}
...
in (columns) for each site (1-12). The full output of the table can be found in 
...
Subsequently, we exclude locus 1 and 13.

---
# Analysing population structure

To investigate patterns of genetic differentiation, we calculated
pairwise F~ST~ values in R and executed Mantel tests and spatial auto-correlation in GenAlEx 
(Excel add-in).
...
allelic richness between hunted and unhunted sites, by modelling the effect
of hunting on these three genetic diversity measures.
...
## First: simple t-test to compare differences in genetic diversity between 
...
Ho_model <- lmerTest::lmer(Hobs ~ hunt + (1|locus) + (1|pop), 
                           data = subset(measures.all, locus != "L13")) # excluding BG20
...
```
```{r calculate Fst, warning = F, message = F, fig.height=5,fig.width=5}
#### Calculate Fst ####
...
```
```{r plot Fst, warning = F, message = F, fig.height=5,fig.width=5}
...
ks <- read.csv("analyses/structure/results/Run_files/output.csv")
...
ggplot(sMLH.all[which(sMLH.all$sex == "m" & sMLH.all$age == "adult")], aes(x=sMLH)) + geom_histogram() +theme_classic() +
  labs(title = "Histogram of sMLH for adult males")

ggplot(sMLH.all[which(sMLH.all$sex == "f" & sMLH.all$age == "adult")], aes(x=sMLH)) + geom_histogram() +theme_classic() +
  labs(title = "Histogram of sMLH for adult females")

ggplot(sMLH.all[which(sMLH.all$age == "chick")], aes(x=sMLH)) + geom_histogram() +theme_classic() +
...
model.both.in <- glmmTMB(migration_ESSc~ hunt_in + Distance + sex + 
...
=======
model.both.in.null <- glmmTMB(migration_ESSc~ Distance + sex + 
                                (1|pop_out) + (1|pop_in), 
                         data = migration_both, 
                         family = Gamma(link = "log"))
...
#with interaction
model.both.in.interaction <- glmmTMB(migration_ESSc~ hunt_in + Distance*sex + 
                                       (1|pop_out) + (1|pop_in), 
                         data = migration_both, 
                         family = Gamma(link = "log"))

model.both.in.interaction.null <- glmmTMB(migration_ESSc~ Distance*sex + 
                                            (1|pop_out) + (1|pop_in), 
                                     data = migration_both, 
                                     family = Gamma(link = "log"))
...
#out
model.both.out <- glmmTMB(migration_ESSc~ hunt_out + Distance + sex + 
                            (1|pop_out) + (1|pop_in), 
                          data = migration_both, 
                          family = Gamma(link = "log"))
model.both.out.null <- glmmTMB(migration_ESSc~ Distance + sex + 
                                 (1|pop_out) + (1|pop_in), 
                          data = migration_both, 
                          family = Gamma(link = "log"))
...
#with interaction
model.both.out.interaction <- glmmTMB(migration_ESSc~ hunt_out + Distance*sex + 
                                        (1|pop_out) + (1|pop_in), 
                          data = migration_both, 
                          family = Gamma(link = "log"))
model.both.out.interaction.null <- glmmTMB(migration_ESSc~ Distance*sex + 
                                             (1|pop_out) + (1|pop_in), 
                                      data = migration_both, 
                                      family = Gamma(link = "log"))
...
#model including interaction outperforms in both
>>>>>>> 27a95f89f214c1c1c603101ead30740590b99881
```
...
=======
```{r plots, warning=F, message=F, fig.height=5,fig.width=5}
>>>>>>> 27a95f89f214c1c1c603101ead30740590b99881
